...
async function readCaseForActor(database, actor, ref) {
  ...
  return Object.freeze({
    ...
    requirementNotes: typeof item.milestonesJson?.icpV2?.requirementNotes === "string" ? item.milestonesJson.icpV2.requirementNotes : null,
    serviceDefinitionStatus: Array.isArray(item.flags) && item.flags.includes("ICP_SERVICE_PENDING") ? "PENDING" : "DEFINED",
    surveyDecisionStatus: Array.isArray(item.flags) && item.flags.includes("ICP_SURVEY_PENDING") ? "PENDING" : "DEFINED",
    ...
    return await database.$transaction(async (tx) => {
      ...
      const pipelineCase = await tx.pipelineCase.create({
        data: {
          ...
          flags: ["ICP_SERVICE_PENDING", "ICP_SURVEY_PENDING", "ICP_VOLUME_PENDING"],
          milestonesJson: {
            icpV2: {
              requirementNotes: command.requirementNotes,
              serviceDefinitionStatus: "PENDING",
              surveyDecisionStatus: "PENDING",
              volumeStatus: "PENDING_SOURCE",
            },
          },
