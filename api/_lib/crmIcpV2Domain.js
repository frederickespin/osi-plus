...
function deriveMode(tenantCountryCode, originCountryCode, destinationCountryCode) {
  if (destinationCountryCode === null) return "LOCAL";
  if (originCountryCode === tenantCountryCode && destinationCountryCode === tenantCountryCode) return "LOCAL";
  if (originCountryCode === tenantCountryCode && destinationCountryCode !== tenantCountryCode) return "EXPORT";
  if (originCountryCode !== tenantCountryCode && destinationCountryCode === tenantCountryCode) return "IMPORT";
...
export function buildCrmIcpV2AtomicPlan(command, authority) {
  ...
  const derivedMode = deriveMode(tenantCountryCode, origin?.countryCode, destination?.countryCode ?? null);
  if (command.mode !== derivedMode) fail("CRM_ICP_ROUTE_INVALID", 400);
