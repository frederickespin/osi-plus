...
El botón **Nuevo ICP** abre un diálogo responsive de tres pasos:
...
El formulario no contiene entrada de volumen ni CBM y el JSON enviado no incluye `estimatedCbm`. La interfaz comunica **Volumen pendiente** y explica que se determinará únicamente desde Survey o desde datos proporcionados con procedencia. El `estimatedCbm: null` usado al calcular el hash pertenece al contrato normalizado del servidor y no se transmite como dato capturado.
