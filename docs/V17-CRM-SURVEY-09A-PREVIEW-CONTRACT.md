...
- El inventario utiliza búsqueda progresiva: prioriza artículos relacionados con el área actual y conserva área y modo después de agregar cada artículo. Al completar `Próximo`, limpia toda la información propia del renglón, deselecciona el catálogo anterior, bloquea pulsaciones duplicadas y devuelve el foco al campo Artículo.
...
- La condición, el icono de cámara y las condiciones especiales se concentran en una sola zona compacta. El icono activa la cámara posterior cuando el dispositivo la ofrece mediante `capture=environment` y utiliza el selector de imagen como alternativa. La fotografía sólo se registra después de recibir un archivo real. Las condiciones especiales usan abreviaturas visibles con su nombre completo accesible y cada artículo admite una nota opcional.
...
- En pantallas móviles, los controles de entrada utilizan un tamaño compatible que evita el acercamiento automático del navegador al seleccionar un campo. El selector de condición y la fila de cámara/condiciones especiales respetan el ancho interior del formulario y nunca se extienden fuera del marco.
...
- La pestaña Firma presenta al cliente un reporte para aceptación con artículos, cantidades, condiciones y totales; excluye precios, costos, márgenes y notas internas. Destaca en bloques separados Huacal, Frágil, Armar/Desarmar, Grúa, AV y SD, mostrando sus fotografías y notas, además de un bloque para artículos que sólo tienen notas o fotos.
- Después de firmar, un icono permite descargar una copia PDF y registra visualmente que el cliente firmó y recibió la copia, con fecha y hora. Este evento es sintético en el Preview; la persistencia auditada en CRM queda reservada al futuro contrato de API.
- Antes de firmar, el cliente puede abrir una vista previa A4 del documento completo. La firma se dibuja con dedo o lápiz digital, exige nombre y relación del firmante y se incorpora como trazos manuscritos al PDF; escribir sólo el nombre no equivale a firmar.
- El PDF usa cabecera corporativa y paginación dinámica. Presenta cliente, empresa, lead account, Booker, servicio, origen, destino, instrucciones, totales métricos e imperiales, inventario agrupado por área, condiciones especiales, evidencias, materiales derivados, declaración, firma, evaluador, referencia y estado del documento.
- El PDF conserva caracteres en español, omite renglones vacíos y excluye costos, precios, márgenes, comisiones y notas administrativas. Una copia firmada identifica la versión aceptada y el evento de entrega deberá auditarse en CRM al conectar la autoridad de servidor.
