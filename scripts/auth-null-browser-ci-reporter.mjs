const EXPECTED = Object.freeze({ chromium: 8, firefox: 8, webkit: 8 });

function projectName(test) {
  return test.parent.project()?.name ?? "unknown";
}

export default class AuthNullBrowserCiReporter {
  discovered = new Map();
  executed = new Map();
  passed = new Map();
  skipped = new Map();
  ...
  onEnd(result) {
    ...
    process.stdout.write(`[auth-null-browser-validation] ${JSON.stringify(summary)}\n`);
    if (errors.length > 0) {
      process.stderr.write(`[auth-null-browser-validation] FAIL ${errors.join("; ")}\n`);
