...
function invariant(condition, message) {
  if (!condition) throw new Error(`CRM01A_LOCAL_TARGET_REJECTED: ${message}`);
}

export function validateCrm01aLocalUrl(raw) {
  invariant(raw, "CRM01A_TEST_DATABASE_URL es obligatoria; no existe fallback");
  let url;
  try { url = new URL(raw); } catch { throw new Error("CRM01A_LOCAL_TARGET_REJECTED: URL inválida"); }
...
export function validateCrm01aDatabaseIdentity(identity, target) {
...
export async function createCrm01aLocalPrisma(raw = process.env.CRM01A_TEST_DATABASE_URL) {
  const target = validateCrm01aLocalUrl(raw);
  ...
  try {
    ...
    `);
    return { prisma, target: validateCrm01aDatabaseIdentity(identity, target) };
  } catch (error) {
