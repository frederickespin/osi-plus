const EXPECTED = Object.freeze({ chromium: 12, firefox: 12, webkit: 12 });

function projectName(test) {
  return test.parent.project()?.name ?? "unknown";
}

export default class Mt01b2bBrowserCiReporter {
  discovered = new Map();
  executed = new Map();
  passed = new Map();
  skipped = new Map();
  ...
  onEnd(result) {
    ...
    process.stdout.write(`[mt01b2b-browser-validation] ${JSON.stringify(summary)}\n`);
    if (errors.length > 0) {
      process.stderr.write(`[mt01b2b-browser-validation] FAIL ${errors.join("; ")}\n`);
