...
export default class V17ConvergenceBrowserCiReporter {
  discovered = new Map();
  executed = new Map();
  passed = new Map();
  skipped = new Map();
  ...
  onEnd(result) {
    ...
    process.stdout.write(`[v17-convergence-browser-validation] ${JSON.stringify(summary)}\n`);
    if (errors.length > 0 || result.status !== "passed") {
      if (errors.length > 0) {
        process.stderr.write(`[v17-convergence-browser-validation] FAIL ${errors.join("; ")}\n`);
      }
