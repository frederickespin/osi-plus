...
rejected("orden CASE antes de REQUEST rechazado", { overrides: { "api/_lib/pipelineCaseDomain.js": domain.replace(/await advisoryLock\(tx, "REQUEST", tenantId, input\.requestId\);\r?\n\s+await advisoryLock\(tx, "CASE", tenantId, input\.caseId\);/, 'await advisoryLock(tx, "CASE", tenantId, input.caseId);\n      await advisoryLock(tx, "REQUEST", tenantId, input.requestId);') } }, /orden de locks/);
