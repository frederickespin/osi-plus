...
check("ruta migrada no puede volver a headers", () => validateMt01b3aSources({ routeSources: new Map([...routeSources].map(([key, value]) => [key, key === "api/k/signal.js" ? `${value}\nconst forged = req.headers['x-osi-role'];` : value])), envExample, authContextSource }), true);
...
check("ruta ICP v2 sin contrato HTTP rechazada", () => validateMt01b3aSources({ routeSources: new Map([...routeSources].map(([key, value]) => [key, key === "api/crm/icp-v2/clients/search.js" ? value.replaceAll("crmIcpV2ApiHttp.js", "legacyOnly.js") : value])), envExample, authContextSource }), true);
