...
try {
  base();
  const migrations = Array.from({ length: 20 }, (_, index) => `prisma/migrations/${String(index).padStart(2, "0")}/migration.sql`);
  ...
  base();
  expectFailure("migración número 21 rechazada", [...safeFiles, "prisma/migrations/20/migration.sql"], {}, /exactamente 20 migraciones/);
  base();
