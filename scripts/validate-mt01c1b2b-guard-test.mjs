...
try {
  base();
  const migrations = Array.from({ length: 19 }, (_, index) => `prisma/migrations/${String(index).padStart(2, "0")}/migration.sql`);
  ...
  base();
  expectFailure("migración número 20 rechazada", [...safeFiles, "prisma/migrations/20/migration.sql"], {}, /exactamente 19 migraciones/);
  base();
