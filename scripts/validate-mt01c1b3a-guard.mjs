...
export function validateMt01c1b3aGuard({ root = process.cwd(), files = trackedFiles(), env = process.env } = {}) {
  ...
  invariant(migrations.length === 14, `MT-01C1B3A conserva exactamente 14 migraciones; encontradas=${migrations.length}`);
  ...
  return { ok: true, runtimeConsumers: 0, migrations: migrations.length, protectedService: "employeeProvisioningExecutor" };
}
