...
export function validateMt01c2b2Guard(root = process.cwd()) {
  ...
  invariant(migrations.length === 15, "deben existir exactamente 15 migraciones");
  invariant(!migrations.some((entry) => /^20260801015/.test(entry.name)), "C2B2 no puede crear migración 16");
  ...
  return { ok: true, migrations: 15, runtimeConsumers: 0, automaticHooks: 0, c2b3Blocked: true, expected: { ...{ clients: 7, projects: 2, leads: 0, pipelineCases: 51, mappedOwners: 39, unassigned: 12 } }, modes: { legacy: true, hybrid: false, tenantSwitch: false, clientV2: false } };
}
