...
try {
  ...
  check("estado actual aprobado", baseline.ok && baseline.migrations === 15 && baseline.preparedConsumers.length === 2);
  ...
  rejected("migración 16 rechazada", { migrations: [...Array.from({ length: 15 }, (_, index) => `m${index}`), "20260801015000_mt01c2b3a"] }, /15 migraciones/);
  rejected("consumidor runtime nuevo rechazado", { extraRuntimeSources: { "api/new-endpoint.js": 'import "../_lib/commercialTenancyWrite.js";' } }, /consumidores preparados/);
  ...
  rejected("actualización de tenant bloqueada", { overrides: { "api/k/project-validate.js": projectValidate.replace('data: { kState: "VALIDATED"', 'data: { tenantId: body.tenantId, kState: "VALIDATED"') } }, /autoridad empresarial/);
  rejected("spread dentro de update K rechazado", { overrides: { "api/k/project-validate.js": projectValidate.replace('data: { kState: "VALIDATED"', 'data: { ...body, kState: "VALIDATED"') } }, /campos permitidos/);
  rejected("ownerId heredado dentro de update rechazado", { overrides: { "api/k/project-validate.js": projectValidate.replace('data: { kState: "VALIDATED"', 'data: { ownerId: body.ownerId, kState: "VALIDATED"') } }, /autoridad empresarial/);
