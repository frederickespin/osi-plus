...
export function validateV17CrmServicesPreviewGuard({ root = process.cwd(), overrides = {} } = {}) {
  ...
  for (const value of ["Resumen", "Servicios", "Survey", "Actividad", "Tareas", "Cotización", "Notas", "Archivos", "Comunicación", "Modo/Alcance", "Servicio principal", "Servicios que incluye", "PRIMARY_SERVICES_BY_SCOPE", "Servicios complementarios", "Otro servicio no catalogado", "pendiente de clasificación", "Catálogo de servicios", "Preparado para análisis", "Seleccionar todos", "Guardar como combo", "Predeterminado", "selected-complementaries-list", "Combos", "Editar", "Desactivar"]) if (!visual.includes(value)) fail(`Preview incompleto: ${value}`);
  ...
  for (const value of ["exclusivamente visual", "No añade migraciones", "Production permanece sin cambios", "no se eliminarán", "códigos independientes", "selector compacto agrupado", "activar o desactivar", "precarga automáticamente", "lista tabular compacta", "Modo/Alcance → Servicio principal → Servicios que incluye", "iconos compactos sin texto adjunto"]) if (!docs.includes(value)) fail(`contrato incompleto: ${value}`);
