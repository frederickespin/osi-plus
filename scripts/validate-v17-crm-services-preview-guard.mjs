...
export function validateV17CrmServicesPreviewGuard({ root = process.cwd(), overrides = {} } = {}) {
  ...
  for (const value of ["Resumen", "Servicios", "Survey", "Actividad", "Tareas", "Cotización", "Notas", "Archivos", "Comunicación", "Servicio principal", "Servicios complementarios", "Otro servicio no catalogado", "pendiente de clasificación", "Catálogo de servicios", "Preparado para análisis"]) if (!visual.includes(value)) fail(`Preview incompleto: ${value}`);
  ...
  for (const value of ["exclusivamente visual", "No añade migraciones", "Production permanece sin cambios", "no se eliminarán", "códigos independientes"]) if (!docs.includes(value)) fail(`contrato incompleto: ${value}`);
  return Object.freeze({ ok: true, migrations: 22, productionChanged: false, apiConsumers: 0, primaryServices: 12, complementaryServices: 13 });
}
