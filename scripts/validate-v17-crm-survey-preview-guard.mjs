...
export function validateV17CrmSurveyPreviewGuard({ root = process.cwd(), overrides = {} } = {}) {
  ...
  for (const value of ["Agenda", "Inventario", "Accesos", "Evidencias", "Revisión", "Detalle técnico", "15 visitas asignadas", "Más próxima", "Salida anticipada", "Registrar llegada", "Cliente confirma llegada", "Puntual · confirmado por cliente", "No selecciona materiales de empaque", "Long carrying", "Motor Logístico", "Firma del cliente visitado", "no precios", "Marítimo", "Aéreo", "Almacenaje", "Resultado automático de recetas administrativas · no editable por el evaluador", "Peso real: pendiente", "Peso cobrable: Cotización", "Publicar resultado de Survey", "Datos sintéticos en este preview"]) if (!visual.includes(value)) fail(`Preview incompleto: ${value}`);
  ...
  return Object.freeze({ ok: true, migrations: 22, productionChanged: false, apiConsumers: 0, previewRoute: "/experience-preview/survey", mobileTerminal: true, assignedVisits: 15, clientArrivalConfirmation: true, evaluatorPackingSelection: false, syntheticData: true });
}
