...
import type { PersonnelPoliciesAccess } from "@/personnel-policies/access";

const ServiceConfigurationAdmin = lazy(() => import("@/crm-services/ServiceConfigurationAdmin"));
...
type Props = Readonly<{
  ...
  personnelPoliciesEnabled?: boolean;
...
export default function AdminTenantMembershipModule({ authorization, effectivePermissions, deniedPermissions, servicesAccess = NO_CRM_SERVICES_ACCESS, invitationEnabled = false, invitationMode = ADMIN_IDENTITY_INVITATION_MODES.DISABLED, costingAccess, costingEnabled = false, communicationsEnabled = false, communicationsAccess, personnelPoliciesEnabled = false, personnelPoliciesAccess, onUnauthorized, api: suppliedApi }: Props) {
  ...
  const logisticsRulesAccess = resolveLogisticsUiAccess(effectivePermissions, deniedPermissions);
  const logisticsAdminEnabled = isLogisticsUiEnabled() && logisticsRulesAccess.canRulesView;

  return <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-7" data-testid="admin-tenant-memberships">
    <div className="mx-auto max-w-7xl">
      <div id="admin-memberships" className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-5">
      ...
      <nav aria-label="Áreas de Administración" className="mt-3 flex flex-wrap gap-2 text-xs font-bold"><a href="#admin-memberships" className="rounded border bg-white px-3 py-1.5 text-slate-700">Acceso y membresías</a>{personnelPoliciesEnabled && personnelPoliciesAccess?.canView && <a href="#admin-personnel-policies" className="rounded border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-indigo-900">Personal y Políticas</a>}{communicationsEnabled && communicationsAccess?.canTemplatesView && <a href="#admin-communications" className="rounded border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-indigo-900">Plantillas y Comunicaciones</a>}{logisticsAdminEnabled && <a href="#admin-logistics-engine" className="rounded border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-indigo-900">Motor Logístico</a>}{costingEnabled && costingAccess?.canRulesView && <a href="#admin-costing" className="rounded border bg-white px-3 py-1.5 text-slate-700">Costos</a>}</nav>
      ...
      {isCrmServicesUiEnabled() && (servicesAccess.canCatalogView || servicesAccess.canPackagesView || servicesAccess.canMaterialsView) && <Suspense fallback={<p className="mt-7 p-6 text-center text-sm text-slate-500">Cargando configuración de Servicios…</p>}><ServiceConfigurationAdmin authorization={authorization} access={servicesAccess} onUnauthorized={onUnauthorized} /></Suspense>}
      ...
      {personnelPoliciesEnabled && personnelPoliciesAccess?.canView && <Suspense fallback={<p className="mt-7 p-6 text-center text-sm text-slate-500">Cargando Personal y Políticas…</p>}><PersonnelPoliciesAdmin authorization={authorization} access={personnelPoliciesAccess} onUnauthorized={onUnauthorized} /></Suspense>}
    </div>
