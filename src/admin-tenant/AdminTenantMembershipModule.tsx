...
export default function AdminTenantMembershipModule({ authorization, effectivePermissions, deniedPermissions, servicesAccess = NO_CRM_SERVICES_ACCESS, invitationEnabled = false, invitationMode = ADMIN_IDENTITY_INVITATION_MODES.DISABLED, costingAccess, costingEnabled = false, onUnauthorized, api: suppliedApi }: Props) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-7" data-testid="admin-tenant-memberships">
    <div className="mx-auto max-w-7xl">
      ...
      {costingEnabled && costingAccess?.canRulesView && <Suspense fallback={<p className="mt-7 p-6 text-center text-sm text-slate-500">Cargando reglas de Costing…</p>}><CostingRulesAdmin authorization={authorization} access={costingAccess} onUnauthorized={onUnauthorized} /></Suspense>}
    </div>
