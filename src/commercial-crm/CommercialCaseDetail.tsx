...
export default function CommercialCaseDetail({ state, onBack, onReload, mutationEnvironmentEnabled, mutationAccess, mutationApi }: Props) {
  ...
  return <section className="min-h-full bg-white" data-testid="commercial-case-detail"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#0070a8]">Seguimiento comercial</p><h1 className="text-xl font-black tracking-tight text-[#003366]">Ficha del Caso</h1></div><div className="flex gap-2">{mutationEnabled && item && <Button size="sm" onClick={() => setEditOpen(true)}><Pencil />Editar</Button>}<Button size="sm" variant="outline" onClick={onBack}><ArrowLeft />Volver al Pipeline</Button></div></div>
