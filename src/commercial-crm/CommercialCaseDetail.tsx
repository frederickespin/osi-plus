...
export default function CommercialCaseDetail({ state, authorization, servicesAccess, logisticsAccess, logisticsEnabled, costingAccess, costingEnabled, onUnauthorized, onBack, onOpenNavigation, onReload, mutationEnvironmentEnabled, mutationAccess, mutationApi }: Props) {
  ...
  const tabs = TABS.filter(([value]) => (value !== "SERVICES" || servicesEnabled) && (value !== "LOGISTICS" || logisticsEnabled && logisticsAccess.canView) && (value !== "COSTING" || costingEnabled && costingAccess.canView));
