  ...
  return <section className="mx-auto max-w-6xl space-y-5 px-4 py-6 sm:px-6 lg:px-8" data-testid="commercial-case-detail">
    ...
    {item && <>
      <header className="rounded-2xl border border-sky-200 bg-gradient-to-br from-white via-sky-50/60 to-cyan-50/30 p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.18em] text-sky-700">Caso Comercial</p><p className="mt-1 truncate font-mono text-xl font-bold text-slate-950">{item.caseCode}</p><p className="mt-1 text-sm text-slate-600">Receptor verificado: {item.client?.displayName || "Sin Client vinculado"}</p></div><div className="flex flex-wrap gap-2"><Badge variant="outline" className={statusClass(item.status)}>{STATUS_LABELS[item.status]}</Badge>{item.mode && <Badge variant="outline">{item.mode}</Badge>}</div></div>
      </header>
