...
import { AlertCircle, ArrowLeft, BellRing, BriefcaseBusiness, Calculator, ClipboardCheck, FileText, History, ListChecks, Menu, MessageSquare, PackageSearch, Paperclip, Pencil, StickyNote } from "lucide-react";
...
type WorkspaceTab = "SUMMARY" | "SERVICES" | "SURVEY" | "LOGISTICS" | "COSTING" | "ACTIVITY" | "TASKS" | "QUOTE" | "NOTES" | "FILES" | "COMMUNICATION";

const TABS = Object.freeze([
  ["SUMMARY", "Resumen", FileText], ["SERVICES", "Servicios", BriefcaseBusiness], ["SURVEY", "Survey", ClipboardCheck], ["LOGISTICS", "Motor Logístico", PackageSearch], ["COSTING", "Costos", Calculator], ["ACTIVITY", "Actividad", History], ["TASKS", "Tareas", ListChecks],
...
export default function CommercialCaseDetail({ state, authorization, servicesAccess, logisticsAccess, logisticsEnabled, costingAccess, costingEnabled, quoteAccess, quoteEnabled, onUnauthorized, onBack, onOpenNavigation, onReload, mutationEnvironmentEnabled, mutationAccess, mutationApi }: Props) {
  ...
  const tabs = TABS.filter(([value]) => (value !== "SERVICES" || servicesEnabled) && (value !== "LOGISTICS" || logisticsEnabled && logisticsAccess.canView) && (value !== "COSTING" || costingEnabled && costingAccess.canView) && (value !== "QUOTE" || quoteEnabled && quoteAccess.canView));
