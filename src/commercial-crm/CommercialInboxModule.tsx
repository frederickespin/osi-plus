...
export default function CommercialInboxModule({ authorization, mutationAccess, role, caseRef, onOpenNavigation, onBack, onOpenCase, onReturnToInbox, onUnauthorized, api: suppliedApi }: Props) {
  ...
  return <section className={`flex ${fullCaseWorkspace ? "min-h-screen" : "min-h-[calc(100vh-4rem)]"} flex-col bg-[#f4f7fb]`} data-testid="commercial-crm-inbox">{!fullCaseWorkspace && <header className="border-b border-slate-200 bg-white"><div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#0070a8]">Control Comercial</p><h1 className="text-xl font-black tracking-tight text-[#003366]">Inbox Comercial</h1><p className="text-xs text-slate-500">{role === "V" ? "Cola personal revalidada por servidor" : "Supervisión tenant-wide"} · CRM relacional</p></div><div className="flex gap-2">{icpUiEnabled && mutationAccess.canCreate ? <Button size="sm" onClick={() => setCreateOpen(true)}><Plus />Nuevo ICP</Button> : mutationEnvironmentEnabled && mutationAccess.canCreate ? <Button size="sm" onClick={() => setCreateOpen(true)}><Plus />Nuevo Caso</Button> : null}<Button size="sm" variant="outline" onClick={onBack}><ArrowLeft />Regresar al Hub</Button></div></div><SummaryStrip value={summary} role={role} /></header>}
    {icpUiEnabled && mutationAccess.canCreate && <IcpIntakeForm open={createOpen} api={icpApi} canCreatePendingDestination={mutationAccess.canCreatePendingDestination} onOpenChange={setCreateOpen} onCommitted={(receipt) => { setIcpReceipt(receipt); setRefresh((value) => value + 1); }} />}
    ...
    {icpReceipt && !fullCaseWorkspace && <Alert className="m-4 mb-0 border-emerald-200 bg-emerald-50 text-emerald-950" data-testid="crm-icp-v2-success"><CheckCircle2 /><AlertTitle>ICP creado correctamente</AlertTitle><AlertDescription>El caso quedó registrado con ruta versión {icpReceipt.routeRevision}. El volumen continúa pendiente hasta Survey o datos proporcionados. <span className="font-mono text-xs">{icpReceipt.caseRef}</span><Button type="button" size="sm" variant="ghost" className="ml-2" onClick={() => setIcpReceipt(null)}>Cerrar</Button></AlertDescription></Alert>}
