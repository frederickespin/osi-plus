...
function CaseRow({ item, onOpen }: { item: CrmPipelineCase; onOpen(caseRef: string): void }) {
  return <tr className="border-b border-slate-100 align-top hover:bg-sky-50/40">
    ...
    <td className="px-4 py-3 text-right"><Button size="sm" variant="outline" onClick={() => onOpen(item.caseRef)}>Abrir ficha</Button></td>
  </tr>;
}

function MobileCase({ item, onOpen }: { item: CrmPipelineCase; onOpen(caseRef: string): void }) {
  return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-sm font-bold">{item.caseCode}</p><p className="text-xs text-slate-500">{formatDate(item.updatedAt)}</p></div><Badge variant="outline">{item.mode}</Badge></div><p className="mt-3 text-sm text-slate-700">{item.originLocation || "Origen pendiente"} → {item.destinationLocation || "Destino pendiente"}</p><div className="mt-3"><Badge data-status={item.status} variant="outline" className={statusClass(item.status)}>{STATUS_LABELS[item.status]}</Badge></div><div className="mt-4 flex items-center justify-between"><span className="text-xs text-slate-600">{item.owner?.displayName || "Sin asignar"}</span><Button size="sm" variant="outline" onClick={() => onOpen(item.caseRef)}>Abrir ficha</Button></div></article>;
}
...
export default function CommercialInboxModule({ authorization, caseRef, onBack, onOpenCase, onReturnToInbox, onUnauthorized, api: suppliedApi }: Props) {
  ...
  return <section className="mx-auto max-w-[1500px] space-y-5 px-4 py-6 sm:px-6 lg:px-8" data-testid="commercial-crm-inbox">
    ...
    {list && list.data.length > 0 && <><div className="hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:block"><div className="overflow-x-auto"><table className="w-full min-w-[980px] border-collapse"><thead className="bg-gradient-to-r from-slate-50 via-sky-50 to-emerald-50"><tr className="border-b text-left text-[11px] uppercase tracking-wide text-slate-500"><th className="px-4 py-3">Caso</th><th className="px-4 py-3">Ruta</th><th className="px-4 py-3">Servicio</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3">Asignación</th><th className="px-4 py-3 text-right">Ficha</th></tr></thead><tbody>{list.data.map((item) => <CaseRow key={item.caseRef} item={item} onOpen={onOpenCase} />)}</tbody></table></div></div><div className="grid gap-3 md:hidden">{list.data.map((item) => <MobileCase key={item.caseRef} item={item} onOpen={onOpenCase} />)}</div></>}
