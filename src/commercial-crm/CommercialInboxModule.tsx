...
import { CrmIcpV2Api } from "@/crm-icp-v2/api";
import { isCrmIcpV2UiEnabled } from "@/crm-icp-v2/clientMode";
...
const MODE_LABELS: Record<PipelineMode, string> = { LOCAL: "LOCAL / NACIONAL", EXPORT: "Exportación", IMPORT: "Importación" };
...
function formatCbm(value: number | null) {
  return value === null || value <= 0 ? "Pendiente de Survey o datos proporcionados" : `${new Intl.NumberFormat("es-DO", { maximumFractionDigits: 2 }).format(value)} m³`;
}
...
export default function CommercialInboxModule({ authorization, mutationAccess, servicesAccess, logisticsAccess, logisticsEnabled, costingAccess, costingEnabled, quoteAccess, quoteEnabled, surveyEnabled, surveySchedulingAccess, role, caseRef, onOpenNavigation, onBack, onOpenCase, onNavigate, onReturnToInbox, onUnauthorized, api: suppliedApi }: Props) {
  ...
  const icpApi = useMemo(() => new CrmIcpV2Api(() => authorization ?? null), [authorization]);
  const icpUiEnabled = isCrmIcpV2UiEnabled();
