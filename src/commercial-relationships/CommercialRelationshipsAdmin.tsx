...
const ALL_TABS = ["Entidades", "Relaciones", "Tarifarios", "Referidos", "Comisiones", "Asociaciones", "Certificaciones"] as const;
...
function workspaceTitle(item: Record<string, unknown>) { return String(item.name || item.tariffCode || item.reference || item.type || (item.association as { displayName?: string })?.displayName || (item.referrer as { displayName?: string })?.displayName || (item.beneficiary as { displayName?: string })?.displayName || item.kind || "Registro versionado"); }
function workspaceSubtitle(item: Record<string, unknown>) { const from = item.validFrom ? new Date(String(item.validFrom)).toLocaleDateString("es-DO") : "inicio abierto"; const to = item.validTo || item.expiresAt ? new Date(String(item.validTo || item.expiresAt)).toLocaleDateString("es-DO") : "sin vencimiento"; return `Versión ${String(item.version || 1)} · ${from} – ${to}`; }
...
export default function CommercialRelationshipsAdmin({ authorization, access, onBack, onUnauthorized }: Props) {
  ...
  const tabs = access.canViewCommissions ? ALL_TABS : ALL_TABS.filter((item) => item !== "Comisiones");
  ...
  const list = tab === "Tarifarios" ? [...(workspace?.tariffs || []), ...(workspace?.pricingAgreements || [])] : tab === "Referidos" ? workspace?.referrals : tab === "Comisiones" ? workspace?.commissions : tab === "Asociaciones" ? workspace?.associations : tab === "Certificaciones" ? workspace?.certifications : [];
