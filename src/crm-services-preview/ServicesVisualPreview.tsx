...
import {
  ...
  Power, Save, StickyNote, Tags, Users, X,
} from "lucide-react";
...
const SERVICE_COMBOS = [
  { code: "MOV_RES_STD", name: "Residencial estándar", primary: "MOV_RES", items: ["PACK_UNPACK", "ASSEMBLY", "MATERIALS", "LOCAL_TRANSPORT"] },
  ...
  { code: "MOV_CORP_FULL", name: "Oficina completa", primary: "MOV_CORP", items: ["PACK_UNPACK", "ASSEMBLY", "LOAD_UNLOAD", "LOCAL_TRANSPORT", "LAST_MILE", "MATERIALS"] },
  { code: "FINE_ART_CARE", name: "Arte con protección integral", primary: "FINE_ART", items: ["CRATING", "RIGGING", "SPECIAL_PROTECTION", "PORT_AIR"] },
] as const;
...
function ComplementaryMultiSelect({ selected, onChange }: Readonly<{ selected: string[]; onChange(next: string[]): void }>) {
  ...
  return <div className="relative mt-1">
    <button type="button" aria-label="Servicios complementarios" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="flex h-10 w-full items-center justify-between rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none focus:border-[#0b5b91] focus:ring-2 focus:ring-sky-100"><span>{selected.length ? `${selected.length} seleccionados` : "Seleccionar complementarios"}</span><ChevronDown className={`h-4 w-4 transition ${open ? "rotate-180" : ""}`} /></button>
    {open && <div className="absolute left-0 right-0 z-30 mt-1 rounded-xl border border-slate-200 bg-white p-3 shadow-xl sm:min-w-[620px]">
...
function CatalogDialog({ open, onClose }: Readonly<{ open: boolean; onClose(): void }>) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-3" data-testid="services-admin-catalog">
    <section role="dialog" aria-modal="true" aria-labelledby="catalog-title" className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="divide-y divide-slate-100">{rows.map(([code, name, category]) => { const disabled = disabledCodes.includes(activeKey(code)); return <div key={`${view}-${code}`} className="px-4 py-3"><div className="grid items-center gap-2 text-sm sm:grid-cols-[125px_1fr_170px_75px_180px]"><code className="text-xs font-bold text-slate-500">{code}</code><strong className={disabled ? "text-slate-400 line-through" : "text-slate-800"}>{name}</strong><span className="text-xs text-slate-500">{category}</span><span className={`w-fit rounded-full px-2 py-1 text-[10px] font-bold uppercase ${disabled ? "bg-slate-200 text-slate-600" : "bg-emerald-100 text-emerald-800"}`}>{disabled ? "Inactivo" : "Activo"}</span><div className="flex justify-end gap-1"><button type="button" aria-label={`Editar ${name}`} onClick={() => setEditingCode(editingCode === code ? "" : code)} className="flex items-center gap-1 rounded-md border border-slate-200 px-2 py-1.5 text-[11px] font-bold text-slate-600 hover:bg-slate-50"><Edit3 className="h-3.5 w-3.5" />Editar</button><button type="button" aria-label={`${disabled ? "Activar" : "Desactivar"} ${name}`} onClick={() => toggleStatus(code)} className={`flex items-center gap-1 rounded-md border px-2 py-1.5 text-[11px] font-bold ${disabled ? "border-emerald-200 text-emerald-700" : "border-rose-200 text-rose-700"}`}><Power className="h-3.5 w-3.5" />{disabled ? "Activar" : "Desactivar"}</button></div></div>{editingCode === code && <div className="mt-3 grid gap-2 rounded-lg border border-sky-200 bg-sky-50 p-3 sm:grid-cols-[1fr_220px_auto]"><label className={labelClass}>Nombre visible<input aria-label={`Nombre visible de ${name}`} className={fieldClass} defaultValue={name} /></label><label className={labelClass}>{view === "COMBO" ? "Servicio principal" : "Grupo para análisis"}<input className={fieldClass} defaultValue={category} /></label><div className="flex items-end"><Button type="button" size="sm" onClick={() => setEditingCode("")}><Save />Guardar cambio</Button></div></div>}</div>; })}</div>
...
function ServicesPanel({ onOpenCatalog }: Readonly<{ onOpenCatalog(): void }>) {
  ...
  const applyCombo = (code: string) => { setCombo(code); const found = SERVICE_COMBOS.find((item) => item.code === code); if (found) changeSelected([...found.items]); };
  return <section role="tabpanel" aria-labelledby="services-heading" className="rounded-xl border border-slate-200 bg-white shadow-sm" data-testid="services-case-panel">
    ...
    <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_280px]"><div className="space-y-4"><div className="grid gap-3 md:grid-cols-3"><label className={labelClass}>Servicio principal *<select aria-label="Servicio principal" className={fieldClass} value={primary} onChange={(event) => { setPrimary(event.target.value); setCombo(""); changeSelected([]); }}><option value="">Seleccionar del catálogo</option>{PRIMARY_SERVICES.map(([code, name]) => <option key={code} value={code}>{name}</option>)}<option value="OTHER">Otro servicio no catalogado</option></select></label><label className={labelClass}>Alcance *<select aria-label="Alcance" className={fieldClass} value={scope} onChange={(event) => { setScope(event.target.value); setSaved(false); }}><option value="">Confirmar alcance</option><option>Local</option><option>Nacional</option><option>Exportación</option><option>Importación</option><option>Sin traslado</option><option>No aplica</option></select></label><label className={labelClass}>Combo sugerido<select aria-label="Combo de complementarios" className={fieldClass} value={combo} disabled={!availableCombos.length} onChange={(event) => applyCombo(event.target.value)}><option value="">{primary && !availableCombos.length ? "Sin combos configurados" : "Aplicar combo opcional"}</option>{availableCombos.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select></label></div>{primary === "OTHER" && <label className={labelClass}>Descripción de otro servicio *<textarea aria-label="Descripción de otro servicio" className="mt-1 min-h-20 w-full rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none focus:ring-2 focus:ring-amber-100" value={otherDescription} onChange={(event) => { setOtherDescription(event.target.value); setSaved(false); }} placeholder="Describe el servicio solicitado para que Administración pueda clasificarlo" /><span className="mt-1 block text-xs font-normal normal-case tracking-normal text-amber-800">Quedará identificado como «Otro pendiente de clasificación».</span></label>}
        <div className="grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><label className={labelClass}>Servicios complementarios<ComplementaryMultiSelect selected={selected} onChange={changeSelected} /></label><Button type="button" size="sm" variant="outline" disabled={!primary || !selected.length} onClick={() => { setSavingCombo(true); setComboSaved(false); }}><Save />Guardar como combo</Button></div>
        {savingCombo && <div className="flex flex-wrap items-end gap-2 rounded-lg border border-sky-200 bg-sky-50 p-3"><label className={`${labelClass} min-w-[240px] flex-1`}>Nombre del nuevo combo<input aria-label="Nombre del nuevo combo" className={fieldClass} value={comboName} onChange={(event) => setComboName(event.target.value)} placeholder="Ej.: Mudanza residencial completa" /></label><Button type="button" size="sm" variant="outline" onClick={() => setSavingCombo(false)}>Cancelar</Button><Button type="button" size="sm" disabled={comboName.trim().length < 3} onClick={() => { setSavingCombo(false); setComboSaved(true); }}><Save />Guardar combo</Button></div>}
        {comboSaved && <p role="status" className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800"><Check className="h-4 w-4" />Combo «{comboName}» preparado para este servicio principal. Podrás editarlo en Administración.</p>}
        {!!selected.length && <div className="flex flex-wrap gap-1.5">{selected.map((code) => { const item = COMPLEMENTARY_SERVICES.find(([itemCode]) => itemCode === code); return item ? <span key={code} className="flex items-center gap-1 rounded-full bg-sky-50 px-2 py-1 text-[11px] font-semibold text-[#003366]">{item[1]}<button type="button" aria-label={`Quitar ${item[1]}`} onClick={() => changeSelected(selected.filter((value) => value !== code))}><X className="h-3 w-3" /></button></span> : null; })}</div>}
      </div>
