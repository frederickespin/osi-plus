...
import {
  Camera, Check, CheckCircle2, ChevronLeft, ChevronRight, FileSignature,
...
function CatalogSettings() {
  return <div className="grid gap-2 rounded-lg border border-indigo-200 bg-indigo-50 p-3 lg:grid-cols-2" data-testid="catalog-configuration"><article className="rounded border border-slate-200 bg-white p-2"><div className="flex items-center justify-between"><strong className="text-xs">Áreas configurables</strong><button type="button" aria-label="Añadir área" className="grid h-7 w-7 place-items-center rounded bg-[#003b70] text-white"><Plus className="h-3.5 w-3.5" /></button></div>{ROOMS.slice(0, 5).map((room) => <div key={room} className="flex items-center justify-between border-t border-slate-100 py-1.5 text-[9px]"><span>{room}</span><span className="text-emerald-700">Activa</span></div>)}</article><article className="rounded border border-slate-200 bg-white p-2"><strong className="text-xs">Artículo y recetas</strong><div className="mt-2 grid grid-cols-[1fr_70px_70px] gap-1"><input defaultValue="Lámpara de pie" className="h-8 rounded border border-slate-300 px-2 text-[10px]" /><input defaultValue="0.22" aria-label="Volumen de catálogo" className="h-8 rounded border border-slate-300 px-2 text-[10px]" /><input placeholder="kg" aria-label="Peso de catálogo" className="h-8 rounded border border-slate-300 px-2 text-[10px]" /></div><p className="mt-1 text-[8px] text-slate-500">Peso vacío: densidad según aéreo o marítimo/terrestre.</p><div className="mt-2 grid grid-cols-2 gap-2 text-[9px]"><span><strong className="block text-emerald-800">Receta local</strong>Mantas · plástico</span><span><strong className="block text-indigo-800">Receta internacional</strong>Cartón · papel · burbuja</span></div></article></div>;
}
...
export function SurveyInventoryWorkspace({ articles, room, mode, modeFilter, onRoomChange, onModeChange, onClearModeFilter, onChange }: InventoryProps) {
  ...
  const selectedCatalog = CATALOG.find((item) => item.id === selectedCatalogId) || CATALOG[0];
  const filtered = articles.filter((item) => item.room === room && (!modeFilter || item.mode === modeFilter));
  const selectedIndex = editingId ? filtered.findIndex((item) => item.id === editingId) : -1;
  ...
  const resetEditor = () => { setEditingId(null); setDeleteConfirm(false); setQuery(""); setQuantity(1); setCondition("Buen estado"); setFlags([]); setNote(""); setPhotoCount(0); setSuggestionsOpen(false); };
  ...
  const save = () => {
    if (needsDamagePhoto && photoCount === 0) return;
  ...
  const navigate = (direction: -1 | 1) => { if (!filtered.length) return; const index = selectedIndex < 0 ? 0 : (selectedIndex + direction + filtered.length) % filtered.length; editArticle(filtered[index]); };
  const pieces = filtered.reduce((sum, item) => sum + item.quantity, 0);

  return <section className="space-y-3" data-testid="survey-inventory"><div className="flex items-center justify-between"><div><h2 className="text-sm font-black text-[#003b70]">Inventario rápido</h2><p className="text-[10px] text-slate-500">Área y modo permanecen activos hasta que los cambies.</p></div><button type="button" onClick={() => setSettingsOpen((value) => !value)} aria-label="Configurar catálogos" className="grid h-8 w-8 place-items-center rounded border border-slate-300"><Settings2 className="h-4 w-4" /></button></div>{settingsOpen && <CatalogSettings />}<div className="rounded-lg border border-slate-200 bg-white p-3"><div className="grid gap-2 md:grid-cols-[145px_125px_minmax(180px,1fr)_auto]"><label><span className="block text-[9px] font-black uppercase text-slate-500">Área</span><select aria-label="Área o habitación" value={room} onChange={(event) => { onRoomChange(event.target.value); resetEditor(); }} className="mt-1 h-9 w-full rounded border border-slate-300 px-2 text-xs">{ROOMS.map((value) => <option key={value}>{value}</option>)}</select></label><label><span className="block text-[9px] font-black uppercase text-slate-500">Modo</span><select aria-label="Modo de traslado" value={mode} onChange={(event) => { onModeChange(event.target.value as SurveyShipmentMode); onClearModeFilter(); }} className="mt-1 h-9 w-full rounded border border-slate-300 px-2 text-xs">{MODES.map((value) => <option key={value}>{value}</option>)}</select></label><label className="relative"><span className="block text-[9px] font-black uppercase text-slate-500">Artículo</span><span className="relative mt-1 block"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input aria-label="Buscar artículo del catálogo" value={query} onChange={(event) => { setQuery(event.target.value); setSuggestionsOpen(true); }} className="h-9 w-full rounded border border-slate-300 pl-9 pr-2 text-xs" placeholder="Empieza a escribir…" /></span>{query && suggestionsOpen && <div className="absolute z-20 mt-1 w-full overflow-hidden rounded border border-slate-200 bg-white shadow-lg" data-testid="article-suggestions">{suggestions.map((item) => <button key={item.id} type="button" onClick={() => { setSelectedCatalogId(item.id); setQuery(item.name); setSuggestionsOpen(false); }} className="flex w-full justify-between border-b border-slate-100 px-3 py-2 text-left text-[10px]"><span><strong className="block">{item.name}</strong><small>{item.frequentRooms.join(" · ")}</small></span>{item.frequentRooms.includes(room) && <span className="text-sky-700">Frecuente aquí</span>}</button>)}{suggestions.length === 0 && <button type="button" onClick={() => { setSettingsOpen(true); setSuggestionsOpen(false); }} className="flex w-full items-center gap-2 px-3 py-2 text-[10px] font-bold text-[#003b70]"><Plus className="h-3.5 w-3.5" />Añadir “{query}” al catálogo</button>}</div>}</label><label><span className="block text-center text-[9px] font-black uppercase text-slate-500">Cantidad</span><div className="mt-1"><QuantityStepper value={quantity} onChange={setQuantity} /></div></label></div><div className="mt-2 grid gap-2 sm:grid-cols-[165px_40px_minmax(0,1fr)]"><label><span className="block text-[9px] font-black uppercase text-slate-500">Condición</span><select aria-label="Condición del artículo" value={condition} onChange={(event) => setCondition(event.target.value as SurveyArticleCondition)} className="mt-1 h-9 w-full rounded border border-slate-300 px-2 text-xs">{CONDITIONS.map((value) => <option key={value}>{value}</option>)}</select></label><div><span className="block text-center text-[9px] font-black uppercase text-slate-500">Foto</span><button type="button" onClick={() => setPhotoCount((value) => value + 1)} aria-label="Añadir foto de comprobación" className={`mt-1 grid h-9 w-9 place-items-center rounded-full border ${photoCount ? "border-emerald-300 bg-emerald-100 text-emerald-800" : needsDamagePhoto ? "border-rose-300 bg-rose-50 text-rose-700" : "border-sky-300 bg-sky-50 text-sky-700"}`}><Camera className="h-4 w-4" /></button></div><div><span className="block text-[9px] font-black uppercase text-slate-500">Condiciones especiales</span><div className="mt-1 flex h-9 items-center gap-1 overflow-x-auto">{FLAG_OPTIONS.map(([flag, short]) => <label key={flag} title={flag} className={`flex shrink-0 items-center gap-1 rounded px-2 py-1.5 text-[9px] font-semibold ${flags.includes(flag) ? "bg-sky-100 text-sky-900" : "bg-slate-100 text-slate-600"}`}><input aria-label={flag} type="checkbox" checked={flags.includes(flag)} onChange={() => toggleFlag(flag)} />{short}</label>)}</div></div></div><div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input aria-label="Nota opcional del artículo" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Nota opcional del artículo…" className="h-9 rounded border border-slate-300 px-3 text-[10px]" /><button type="button" onClick={save} disabled={needsDamagePhoto && photoCount === 0} className="flex h-9 items-center justify-center gap-1 rounded bg-[#003b70] px-4 text-[10px] font-bold text-white disabled:bg-slate-300">{editingId ? "Actualizar" : "Próximo"}<ChevronRight className="h-3.5 w-3.5" /></button></div>{needsDamagePhoto && photoCount === 0 && <p role="alert" className="mt-2 text-[9px] font-bold text-rose-700">La condición seleccionada requiere una fotografía.</p>}{showMeasurements && <div className="mt-2 rounded border border-amber-200 bg-amber-50 p-2" data-testid="conditional-measurements"><div className="flex items-center gap-1 text-[9px] font-black text-amber-900"><Ruler className="h-3.5 w-3.5" />Medidas activadas</div><div className="mt-2 grid grid-cols-3 gap-2">{[["Largo", "120", "47.2"], ["Ancho", "60", "23.6"], ["Alto", "80", "31.5"]].map(([label, cm, inch]) => <label key={label}><input aria-label={`${label} del artículo`} defaultValue={cm} className="h-8 w-full rounded border border-slate-300 px-2 text-xs" /><small className="text-[8px] text-slate-500">cm · {inch} in</small></label>)}</div></div>}</div><div className="overflow-hidden rounded-lg border border-slate-200 bg-white"><header className="flex items-center justify-between gap-2 bg-slate-100 px-3 py-2"><strong className="text-[10px] text-[#003b70]">{room} · {filtered.length} {filtered.length === 1 ? "renglón" : "renglones"} · {pieces} {pieces === 1 ? "pieza" : "piezas"}{modeFilter ? ` · ${modeFilter}` : ""}</strong><span className="flex items-center gap-1">{modeFilter && <button type="button" onClick={onClearModeFilter} className="flex items-center gap-1 rounded bg-white px-2 py-1 text-[8px]"><X className="h-3 w-3" />Quitar modo</button>}<button type="button" aria-label="Artículo anterior" onClick={() => navigate(-1)} className="grid h-7 w-7 place-items-center rounded border border-slate-300 bg-white"><ChevronLeft className="h-3.5 w-3.5" /></button><button type="button" aria-label="Artículo siguiente" onClick={() => navigate(1)} className="grid h-7 w-7 place-items-center rounded border border-slate-300 bg-white"><ChevronRight className="h-3.5 w-3.5" /></button></span></header><div className="grid grid-cols-[minmax(120px,1fr)_38px_82px_18px] px-3 py-1.5 text-[8px] font-black uppercase text-slate-500 sm:grid-cols-[minmax(160px,1fr)_45px_110px_100px_18px]"><span>Artículo</span><span>Cant.</span><span>Volumen</span><span className="hidden sm:block">Peso</span><span /></div>{filtered.map((item) => <button key={item.id} type="button" onClick={() => editArticle(item)} className={`grid w-full grid-cols-[minmax(120px,1fr)_38px_82px_18px] items-center border-t border-slate-100 px-3 py-2 text-left text-[10px] sm:grid-cols-[minmax(160px,1fr)_45px_110px_100px_18px] ${editingId === item.id ? "bg-sky-50" : "bg-white"}`}><span className="min-w-0"><strong className="block truncate">{item.name}</strong><small className={`${item.condition === "Averiado" || item.condition === "Daño preexistente" ? "font-bold text-rose-700" : "text-slate-500"}`}>{item.condition}{item.note ? " · nota" : ""}{item.photoCount ? ` · ${item.photoCount} foto` : ""}</small></span><strong>{item.quantity}</strong><span>{(item.volumeM3 * item.quantity).toFixed(2)} m³<small className="block text-[8px] text-slate-500">{cubicFeet(item.volumeM3 * item.quantity).toFixed(1)} ft³</small></span><span className="hidden sm:block">{(item.weightKg * item.quantity).toFixed(0)} kg<small className="block text-[8px] text-slate-500">{pounds(item.weightKg * item.quantity).toFixed(0)} lb</small></span><ChevronRight className="h-3.5 w-3.5 text-slate-400" /></button>)}{filtered.length === 0 && <p className="px-3 py-5 text-center text-[10px] text-slate-500">No hay artículos en esta área y modo.</p>}</div>{editingId && <div className="flex items-center justify-end gap-2 rounded border border-sky-200 bg-sky-50 p-2 text-[10px]"><span className="mr-auto font-bold text-sky-900">Editando el artículo seleccionado</span>{deleteConfirm ? <><button type="button" onClick={() => setDeleteConfirm(false)} className="rounded px-2 py-1">Cancelar</button><button type="button" onClick={() => { onChange(articles.filter((item) => item.id !== editingId)); resetEditor(); }} className="rounded bg-rose-700 px-2 py-1 font-bold text-white">Confirmar eliminación</button></> : <><button type="button" onClick={resetEditor} aria-label="Cancelar edición" className="grid h-7 w-7 place-items-center rounded border border-slate-300 bg-white"><X className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setDeleteConfirm(true)} aria-label="Eliminar artículo" className="grid h-7 w-7 place-items-center rounded border border-rose-300 bg-white text-rose-700"><Trash2 className="h-3.5 w-3.5" /></button></>}</div>}<p className="text-[10px] text-slate-500">El listado se filtra por el área activa. El evaluador no selecciona materiales de empaque.</p></section>;
}
