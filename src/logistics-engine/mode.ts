...
export function resolveLogisticsUiMode(environment: Readonly<Record<string, unknown>> = import.meta.env, hostname = window.location.hostname): LogisticsUiMode {
  ...
  return value === "PREVIEW_REHEARSAL" && environment.VITE_LOGISTICS_ENGINE_UI_BATCH === "V17-LOGISTICS-ENGINE-07A-PREVIEW" && environment.VITE_VERCEL_ENV === "preview" && environment.VITE_VERCEL_GIT_COMMIT_REF === "feature/v17-logistics-engine" ? value : "DISABLED";
}
