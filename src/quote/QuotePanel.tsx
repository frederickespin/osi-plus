...
function CreateProposal({ caseRef, authorization, costing, position, onCreated }: { caseRef: string; authorization?: string; costing: CostingRevision; position: number; onCreated(): void }) {
  ...
  const submit = async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "QUOTE_REQUEST_FAILED"); }
...
export default function QuotePanel({ caseRef, authorization, access, onUnauthorized }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (code.includes("UNAUTHORIZED")) onUnauthorized(); else setError(code);
  ...
  return <section role="tabpanel" data-testid="quote-panel" className="border border-slate-200 bg-white"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[#0070a8]">Cotización tenant-first</p><h2 className="text-lg font-black text-[#003366]">Propuestas comerciales</h2></div><Button size="sm" variant="outline" onClick={() => void load()}><RefreshCw />Actualizar</Button></header>
    ...
    {!loading && value && <><div className="flex gap-2 overflow-x-auto border-b border-slate-200 p-3">{value.proposals.map((proposal) => <ProposalHeader key={proposal.proposalRef} proposal={proposal} active={proposal.proposalRef === activeRef} onClick={() => setActiveRef(proposal.proposalRef)} />)}{access.canCreate && costing && nextPosition && <CreateProposal caseRef={caseRef} authorization={authorization} costing={costing} position={nextPosition} onCreated={() => void load()} />}{!costing && value.proposals.length === 0 && <p className="p-2 text-xs text-amber-800">Se requiere una CostingRevision publicada.</p>}</div>
      {active ? <div><div className="grid gap-3 border-b border-slate-200 p-4 md:grid-cols-4"><div><span className="text-[10px] font-bold uppercase text-slate-500">Propuesta</span><strong className="block text-sm">{active.proposalName}</strong></div><div><span className="text-[10px] font-bold uppercase text-slate-500">Referencia</span><strong className="block font-mono text-sm">{active.reference}</strong></div><div><span className="text-[10px] font-bold uppercase text-slate-500">Cantidad / Total</span><strong className="block text-sm">{active.lines.length} · {money(active.totals.totalQuotedPrice, active.currency)}</strong></div><div><span className="text-[10px] font-bold uppercase text-slate-500">Estado</span><Badge>{active.state}</Badge></div></div><QuoteLines proposal={active} internal={access.canViewInternalCost} /><footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 p-3"><p className="text-xs text-slate-500">Vigente hasta {active.validUntil} · Revisión {active.revision}</p><div className="flex flex-wrap gap-2">{access.canPublish && active.state === "DRAFT" && <Button size="sm" onClick={() => void quoteApi.publish(authorization, active.proposalRef, active.revision).then(load)}><Check />Publicar</Button>}{access.canSend && active.state === "READY" && <Button size="sm" onClick={() => void quoteApi.send(authorization, { proposalRef: active.proposalRef, expectedRevision: active.revision, channel: "MANUAL", recipient: { kind: "RECIPIENT_ON_FILE", displayName: null, reference: null, present: true }, evidenceRef: null }).then(load)}><Send />Registrar envío</Button>}{access.canRecordDecision && active.state === "SENT" && <><Button size="sm" onClick={() => void quoteApi.decision(authorization, { proposalRef: active.proposalRef, expectedRevision: active.revision, decision: "ACCEPTED", method: "EVIDENCE_ON_FILE", decidedBy: { kind: "CLIENT_REPRESENTATIVE", displayName: null, reference: null, present: true }, evidenceRef: "CLIENT-ACCEPTANCE-EVIDENCE", reason: null }).then(load)}><Check />Registrar aceptación</Button><Button size="sm" variant="outline" onClick={() => void quoteApi.decision(authorization, { proposalRef: active.proposalRef, expectedRevision: active.revision, decision: "REJECTED", method: "EVIDENCE_ON_FILE", decidedBy: { kind: "CLIENT_REPRESENTATIVE", displayName: null, reference: null, present: true }, evidenceRef: "CLIENT-REJECTION-EVIDENCE", reason: "Decisión del cliente registrada" }).then(load)}>Registrar rechazo</Button></>}</div></footer></div> : <p className="p-8 text-center text-sm text-slate-500">Cree una propuesta desde la revisión económica publicada.</p>}</>}
  </section>;
}
