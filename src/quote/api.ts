...
type QuoteCommercialParty = null | Readonly<{ kind: string; reference: string; displayName: string; version: number; validFrom: string | null; validUntil: string | null }>;
export type QuoteProposal = Readonly<{ proposalRef: string; reference: string; position: number; state: string; revisionRef: string; revision: number; proposalName: string; costingRevisionRef: string; currency: string; issueDate: string; validUntil: string; commercialContext: Readonly<{ company: QuoteCommercialParty; leadAccount: QuoteCommercialParty; booker: QuoteCommercialParty; tariff: QuoteCommercialParty; associations: readonly string[]; referral: QuoteCommercialParty; commissionContext: unknown | null }>; payer: Readonly<{ kind: string; reference: string; displayName: string; sourceVersion: number; validFrom: string | null; validUntil: string | null }>; totals: Record<string, number>; lines: readonly QuoteLine[]; issues: readonly { code: string; message: string }[]; logicalSha256: string }>;
