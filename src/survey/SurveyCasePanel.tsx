...
type Props = Readonly<{ caseRef: string; authorization?: string; access: SurveySchedulingUiAccess; communicationsEnabled?: boolean; communicationsAccess?: CommunicationsAccess; logisticsSummaryEnabled?: boolean; onNavigate(pathname: string): void; onUnauthorized(): void }>;
...
export default function SurveyCasePanel({ caseRef, authorization, access, communicationsEnabled = false, communicationsAccess, logisticsSummaryEnabled = false, onNavigate, onUnauthorized }: Props) {
  ...
  return <section role="tabpanel" data-testid="survey-scheduling-workspace" className="border border-slate-200 bg-white">
    ...
    {!loading && workspace && <div className="grid min-w-0 gap-0 xl:grid-cols-[minmax(0,1.25fr)_minmax(300px,.75fr)]">
      <div className="min-w-0 divide-y">
        ...
        <section className="grid gap-4 p-4 sm:grid-cols-2"><div><h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-500"><MapPin className="h-4 w-4" />Visit Fee</h3>{!access.canViewFee ? <p className="mt-2 text-sm text-slate-500">Sin permiso de consulta.</p> : workspace.visitFee ? <div className="mt-2 space-y-1 text-sm"><p className="font-bold text-slate-900">{workspace.visitFee.disposition === "FREE" ? "Sin costo" : workspace.visitFee.disposition === "WAIVED" ? "Exonerada" : workspace.visitFee.suggestedAmount == null ? "Pendiente de cálculo" : `${workspace.visitFee.currency} ${workspace.visitFee.suggestedAmount.toLocaleString("es-DO")}`}</p><p className="text-xs text-slate-500">Comunicación: {workspace.visitFee.communicationStatus} · aprobación: {workspace.visitFee.approvalStatus} · pago: {workspace.visitFee.paymentStatus}</p></div> : <p className="mt-2 text-sm text-slate-500">Pendiente de cálculo por Motor Logístico y Costing. Scheduling no estima importes.</p>}</div><div><h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-wide text-slate-500"><MessageSquareText className="h-4 w-4" />Comunicación PIC</h3><p className="mt-2 text-sm text-slate-600">Plantillas publicadas y destinatarios explícitos para cliente y evaluador.</p><p className="mt-2 text-[11px] text-slate-500">PREPARED no envía mensajes externos.</p></div></section>
        {communicationsEnabled && communicationsAccess && workspace.assignment && <CommunicationPanel caseRef={caseRef} authorization={authorization} access={communicationsAccess} context="SCHEDULING" surveyAssignmentRef={workspace.assignment.assignmentRef} onUnauthorized={onUnauthorized} />}
