...
export function resolveToolsEquipmentUiMode(environment: Readonly<Record<string, unknown>> = import.meta.env, hostname = window.location.hostname): ToolsEquipmentUiMode {
  ...
  const preview = value === "PREVIEW_REHEARSAL" && environment.VITE_TOOLS_EQUIPMENT_UI_BATCH === "V17-TOOLS-EQUIPMENT-06A-PREVIEW" && environment.VITE_VERCEL_ENV === "preview" && (environment.VITE_VERCEL_GIT_COMMIT_REF === "feature/v17-tools-equipment" || isV17ConsolidatedPreviewBranch(environment.VITE_VERCEL_GIT_COMMIT_REF));
