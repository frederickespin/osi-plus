...
test("catálogo compacto edita draft y genera preview sintético", async ({ page }) => {
  ...
  await expect(page.getByText("Variables declaradas · catálogo v1")).toBeVisible();
  await page.getByRole("button", { name: "Preview sintético" }).click();
...
test("Scheduling y Quote montan comunicación ligada a assignment y revisión exactos", async ({ page }) => {
  ...
  await page.route("**/api/communications/records?*", (route) => route.fulfill({ status: 200, contentType: "application/json", headers, body: JSON.stringify({ ok: true, data: [] }) }));
