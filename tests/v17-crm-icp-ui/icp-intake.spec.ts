...
async function authenticate(page: Page, options: { role?: string; permissions?: string[]; deniedPermissions?: string[] } = {}) {
  ...
  await page.addInitScript(({ roleValue, membershipRef }) => {
    localStorage.setItem("osi-plus.token", "synthetic.icp.ui.token");
    localStorage.setItem("osi-plus.session", JSON.stringify({
      name: "Actor sintético",
      role: roleValue,
      membershipRef,
      memberships: [{ membershipRef, tenantName: "Tenant ICP sintético", role: roleValue, preferred: true }],
    }));
  }, { roleValue: role, membershipRef: MEMBERSHIP_REF });
    ...
    return route.fulfill({
      ...
      body: JSON.stringify({
        ...
        user: {
          ...
          permissions,
          deniedPermissions,
          membership: { membershipRef: MEMBERSHIP_REF, tenantName: "Tenant ICP sintético", role },
          memberships: [{ membershipRef: MEMBERSHIP_REF, tenantName: "Tenant ICP sintético", role, preferred: true }],
        },
      }),
    });
  });
