...
test("Preview de Servicios muestra complementarios agrupados, combos y catálogo administrable", async ({ page }) => {
  ...
  await expect(page.getByLabel("Servicio principal")).toBeDisabled();
  await page.getByLabel("Modo/Alcance").selectOption({ label: "Local" });
  await expect(page.getByLabel("Servicio principal")).toBeEnabled();
  await expect(page.getByLabel("Servicio principal").locator('option[value="CUSTOMS"]')).toHaveCount(0);
  await page.getByLabel("Servicio principal").selectOption("MOV_RES");
  await expect(page.getByLabel("Servicios que incluye")).toHaveValue("MOV_RES_STD");
  ...
  await page.getByLabel("Servicio principal").selectOption("MOV_RES");
  await expect(page.getByLabel("Servicios que incluye")).toHaveValue("CUSTOM-MOV_RES");
  await expect(page.getByTestId("selected-complementaries-list").getByRole("button")).toHaveCount(5);
