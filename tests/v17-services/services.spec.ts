...
async function session(page: Page, denied = false) {
  const permissions = denied ? ["pipeline:view"] : ["pipeline:view", "services:case:view", "services:case:update", "services:case-config:view", "services:case-config:update"];
...
async function crm(page: Page) {
  ...
  await page.route("**/api/crm/icp-v2/pipeline-cases/**", (route) => route.fulfill({ status: 409, contentType: "application/json", headers: privateHeaders, body: JSON.stringify({ ok: false, error: "CRM_ICP_V2_DISABLED" }) }));
  await page.route(`**/api/crm/services/configurations/${CASE_REF}`, (route) => route.fulfill({ status: 200, contentType: "application/json", headers: privateHeaders, body: JSON.stringify({ ok: true, data: { caseRef: CASE_REF, mode: "LOCAL", current: null, packages: [], materialPolicies: [], precedence: ["CASE_OVERRIDE", "COMMERCIAL_AGREEMENT", "SERVICE_PACKAGE", "SERVICE_MODE_DEFAULT", "MANUAL_RESOLUTION"] } }) }));
}
...
test("configuración final versiona paquete, materiales y duración sin IDs internos", async ({ page }) => {
  await session(page); await crm(page); const selectionRef = "638f6d8f-8d11-4f39-8a2d-1b6c7e8f9012"; const packageRef = "738f6d8f-8d11-4f39-8a2d-1b6c7e8f9012"; const packageVersionRef = "838f6d8f-8d11-4f39-8a2d-1b6c7e8f9012"; const policyRef = "938f6d8f-8d11-4f39-8a2d-1b6c7e8f9012"; const policyVersionRef = "a38f6d8f-8d11-4f39-8a2d-1b6c7e8f9012"; let saved: Record<string, unknown> | null = null; let configurationReads = 0;
  const selected = { ...workspace(), selection: { selectionRef, revision: 1, mode: "LOCAL", source: "MANUAL", defaultCombinationRef: DEFAULT_REF, primary: { serviceRef: PRIMARY_LOCAL, code: "MOV_LOCAL", name: "Mudanza local", category: "Mudanzas", catalogVersion: 1, source: "MANUAL" }, complementaries: [{ serviceRef: PACKING, code: "PACKING", name: "Empaque", category: "Mudanzas", catalogVersion: 1, source: "MANUAL" }], otherServices: [], historyCount: 1 } };
  await page.route(`**/api/crm/services/cases/${CASE_REF}`, (route) => route.fulfill({ status: 200, contentType: "application/json", headers: privateHeaders, body: JSON.stringify({ ok: true, data: selected }) }));
  await page.route(`**/api/crm/services/configurations/${CASE_REF}`, async (route) => { if (route.request().method() === "POST") { saved = route.request().postDataJSON() as Record<string, unknown>; return route.fulfill({ status: 200, contentType: "application/json", headers: privateHeaders, body: JSON.stringify({ ok: true, data: { replayed: false } }) }); } configurationReads += 1; return route.fulfill({ status: 200, contentType: "application/json", headers: privateHeaders, body: JSON.stringify({ ok: true, data: { caseRef: CASE_REF, mode: "LOCAL", current: null, packages: [{ packageRef, versionRef: packageVersionRef, code: "LOC-MOVE-STD", name: "Mudanza local estándar", description: null, category: "MUDANZA", tags: [], state: "PUBLISHED", version: 1, validFrom: null, validTo: null, modes: [{ modeRef: "b38f6d8f-8d11-4f39-8a2d-1b6c7e8f9012", code: "LOCAL", name: "Local / Nacional" }], primary: { serviceRef: PRIMARY_LOCAL, code: "MOV_LOCAL", name: "Mudanza local" }, complementaries: [{ serviceRef: PACKING, code: "PACKING", name: "Empaque" }], requirements: [] }], materialPolicies: [{ policyRef, versionRef: policyVersionRef, code: "LOCAL-REUSE", name: "Materiales reutilizables", version: 1, state: "PUBLISHED", packageVersionRef, serviceRef: null, modeRef: null, preferenceCode: "REUSABLE_ALLOWED", standardCode: "STANDARD", validFrom: null, validTo: null, lines: [] }], precedence: ["CASE_OVERRIDE", "COMMERCIAL_AGREEMENT", "SERVICE_PACKAGE", "SERVICE_MODE_DEFAULT", "MANUAL_RESOLUTION"] } }) }); });
  await page.goto(`/commercial/cases/${CASE_REF}`); await page.getByRole("tab", { name: "Servicios" }).click(); const configuration = page.getByTestId("case-service-configuration"); await expect(configuration).toBeVisible(); await configuration.getByLabel("Paquete publicado").selectOption(packageVersionRef); await configuration.getByLabel("Política de materiales").selectOption(policyVersionRef); await configuration.getByLabel("Duración asumida (horas)").fill("8"); await configuration.getByRole("button", { name: "Confirmar configuración" }).click(); await expect.poll(() => saved).not.toBeNull(); await expect.poll(() => configurationReads).toBeGreaterThanOrEqual(2); expect(saved).toMatchObject({ expectedRevision: 0, serviceSelectionRef: selectionRef, packageVersionRef, materialPolicyVersionRef: policyVersionRef, duration: { hours: 8 }, source: "PACKAGE", overrides: [] }); expect(JSON.stringify(saved)).not.toMatch(/tenantId|membershipId|userId|clientId|stock|cost/i);
});
